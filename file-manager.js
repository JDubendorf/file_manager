...
function removeFile(fileName) {
	fs.unlink(fileName, function(err) {
		if (err) {
			console.log("Unable to remove " + fileName);
		} else {
...
function findReplaceWord(fileName, wordSearch, wordReplace) {
	fs.readFile(fileName, function (err,data) {
		if (err) {
			console.log("unable to read file");
		} else {
			var text = data.toString();
			var newText = text.replace((wordSearch), wordReplace);	// https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/RegExp/@@replace
	...
	// * find a line in a file  //fs.read  // find every line with an instance of a word
...
function findLine (fileName, wordSearch) {
	fs.readFile(fileName, function(err, data) {
		if (err) {
			console.log("unable to read file");
		} else {
